...
:cambia
set ip=%a%.%b%.%c%.%d%
FOR /F "TOKENS=3 DELIMS=	" %%F IN ('reg query "\\%ip%\HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\NetworkCards" /s ^| find /i "ServiceName"') DO (reg add "\\%ip%\HKLM\SYSTEM\CurrentControlSet\Services\NetBT\Parameters\Interfaces\Tcpip_%%F" /v NetbiosOptions /t REG_DWORD /d 2 /f > nul)
set rsp=%errorlevel%
